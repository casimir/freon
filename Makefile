...
server-headless::
	...
	cd server && \
	CGO_ENABLED=1 go build -o build/freon-headless -ldflags="-X 'buildinfo.Version=${VERSION}'"

server-full:: ui
	mkdir -p server/build
	cp -r ui/build/web server/ui/statics
	cd server && \
	CGO_ENABLED=1 go build -o build/freon -tags embed -ldflags="-X 'buildinfo.Version=${VERSION}'"
